...
##ggplot version
# ggplot(data = as.data.frame(result),aes(x=result))+geom_histogram() +geom_vline(xintercept=5, color="blue", size=0.5)
...
exploglik<- function(rate,x){
    -sum(x=x,dexp(rate = rate,log = TRUE))
}

# Code the newton-raphson method
nr <- function(rate_guess,data,tolerance){
